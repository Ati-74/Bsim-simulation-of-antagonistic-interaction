...
public class AtiMonoCultureTicker extends BSimTicker {
    ...
    final ArrayList<AtiMonoCultureBacterium> bac_bornbacSuscp;
    final ArrayList<AtiMonoCultureBacterium> bac_deadbacSuscp;
    ...
    public AtiMonoCultureTicker(BSim sim, ArrayList<AtiMonoCultureBacterium> suscp_bac,
        ...
        AtiMonoCultureTicker.bacRng = bacRng; //random number generator
    ...
    public void growSuscp( ArrayList<AtiMonoCultureBacterium> bac, ArrayList<AtiMonoCultureBacterium> bacBorn ) {
    	Random bacRng = new Random(); 			// Random number generator
    	bacRng.setSeed(50); 					// Initializes random number generator

        for (AtiMonoCultureBacterium b : bac) { 				// Loop over bac array
            b.grow();

            // Divide if grown past threshold
            if (b.L >= b.L_th) {
            	AtiMonoCultureBacterium daughter = b.divide();
        ...
        for ( AtiMonoCultureBacterium b : bacBorn ) {
    ...
    public void removeBacteria( BSim sim, ArrayList<AtiMonoCultureBacterium> bac, ArrayList<AtiMonoCultureBacterium> bac_dead ) {

        for (AtiMonoCultureBacterium b : bac) {
    ...
    public void tick() {
        ...
        for (AtiMonoCultureBacterium b : suscp_bac) {
            b.lifetime++;
        }
        ...
        if (WITH_GROWTH) {
            ...
            growSuscp( suscp_bac, bac_bornbacSuscp );		// For  bacteria population
            ...
            removeBacteria( sim, suscp_bac, bac_deadbacSuscp );		// For bacteria population
